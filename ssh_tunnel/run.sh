...
command_args="-M 0 -N -q -o StrictHostKeyChecking=no -o ServerAliveInterval=${ALIVE_INTERVAL} -o ServerAliveCountMax=${ALIVE_COUNT} -o TCPKeepAlive=yes ${USERNAME}@${HOSTNAME} -p ${SSH_PORT} -i ${KEY_PATH}/autossh_key"
